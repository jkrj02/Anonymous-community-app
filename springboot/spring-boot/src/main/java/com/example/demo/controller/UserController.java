...
public class UserController {
    ...
    @PostMapping("course")//添加课程
    public ResponseBody addcourse(@RequestBody course user) {
        if (courseService.exists(user)) {
            course u = courseService.findByNameAndPassword(user);

            return new ResponseBody(u != null ? ResponseCode.SIGN_IN_SUCCESS : ResponseCode.SIGN_IN_FAILED, u != null ? u.getId() : "");
        }
        return new ResponseBody(courseService.insert(user) ? ResponseCode.SIGN_UP_SUCCESS : ResponseCode.SIGN_UP_FAILED, "");
    }
